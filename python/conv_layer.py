...
def conv_layer_forward(input_data, layer, param):
    ...
    # col_img = im2col_conv_batch(input_n, layer, h_out, w_out)
    # output_img = np.zeros((num,h_out,w_out,batch_size))
    # for b in range(batch_size):
    #     for n in range(num):
    #         for row in range(h_out):
    #             for column in range(w_out):
    #                 output_img[n,row,column,b] = (np.dot(W[:,n].flatten(), col_img[:,row*w_out+column, b].flatten())) + bias[n]

    # output['data'] = output_img.reshape(h_out * w_out * num, batch_size)

    W = param['w'].reshape((k*k*c, num))
    bias = param['b'].reshape((1,num))

    col_img = im2col_conv_batch(input_n, layer, h_out, w_out)
    output_img = np.zeros((h_out,w_out,num,batch_size))

    for b in range(batch_size):
        temp = np.dot(col_img[:, :, b].T, W) + bias
        output_img[:,:,:,b] = temp.reshape((h_out, w_out, num))

    output['data'] = np.transpose(output_img,(2,0,1,3)).reshape((h_out * w_out * num, batch_size))
