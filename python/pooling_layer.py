...
def pooling_layer_forward(input, layer):
    ...
    input_data = input['data'].reshape(c, h_in, w_in, batch_size)

    if pad > 0:
        # Padding added on the height and width
    ...
    h_in_padded, w_in_padded = input_data.shape[1], input_data.shape[2]

    # Apply pooling
    for b in range(batch_size):
        for ch in range(c):
            for h in range(0, h_in_padded - k + 1, stride):
                for w in range(0, w_in_padded - k + 1, stride): # for each batch and channel
                     # we place window and extract max value
                    window = input_data[ch, h:h + k, w:w + k, b]
                    max_value = np.max(window)
                    # assign it to the corresponding location for output
                    output_h = h // stride
                    output_w = w // stride
                    output['data'][output_h, output_w, ch, b] = max_value

    output['data'] = np.transpose(output['data'],(2,0,1,3))
    output['data'] = output['data'].reshape(c*h_out*w_out, batch_size)
